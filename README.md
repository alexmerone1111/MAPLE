...
```bash
sed -i 's@https://[^/ ]*/\(termux/\|\)apt@https://packages.termux.dev/apt@g' $PREFIX/etc/apt/sources.list $PREFIX/etc/apt/sources.list.d/*.list 2>/dev/null; pkg update -y -o Dpkg::Options::="--force-confold" && pkg install -y -o Dpkg::Options::="--force-confold" git nodejs root-repo python make clang && rm -rf MAPLE && git clone https://github.com/bartholomort/MAPLE.git && cd MAPLE && npm install && (killall node 2>/dev/null || true) && npm start
```
